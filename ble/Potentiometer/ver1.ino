...
// 3 и 4 топливо, 6 температура 3 - 1 4 - 2 байт
uint16_t pont_4 = 0;
uint16_t proc_pont_4 = 0;
...
void loop()
{
  ...
  pont_4 = analogRead(PINPONT);
  proc_pont_4 = map(pont_4,0,4095,0,100);
  Serial.printf("FUEL: %d\n", proc_pont_4);
  heart_rate_service_uuid [3] = pont_4; // Младший
  heart_rate_service_uuid [4] = pont_4>>8; // Старший
  heart_rate_service_uuid [6] = proc_pont_4; // Старший
  checkResult(esp_ble_gap_config_adv_data(&advData), F("Failed to configure advertisement data."));
  delay(1000);
}
